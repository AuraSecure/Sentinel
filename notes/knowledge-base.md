...
- **Dev/test toolchain — answered.** Python + pytest, living in `src/detector/`. State machine, parser, and fusion logic are prototyped there against synthetic scenarios; validated logic gets ported to firmware later. Firmware language/framework (Arduino, PlatformIO, ESP-IDF) is still **undecided — needs your call before firmware work starts**, since it shapes the whole next phase.
- **What "privacy-first" means in practice — proposed default, unconfirmed.** Audio is processed for features only (impulse/energy) and never recorded or stored as raw audio, with no cloud upload. `audio_pcm.py` already follows this (raw PCM samples never leave the function that reduces them to an `AudioFrame`) — flagging here so it's a deliberate choice, not an accident of how the code happened to get written.
- **Alert destination — proposed default, unconfirmed.** Assume v1 targets local-only output (serial log, then LED/buzzer) with no caregiver/monitoring-service integration, matching the "starts as serial log / LED / buzzer, later extends to network or app output" line already in the architecture plan above. Network/app/caregiver-facing alerts are a v2+ decision, not something to design for yet.
- **Occupant/environment assumptions — proposed default, unconfirmed.** Single adult occupant, single room. LD2410C radar will pick up pets and can't inherently identify "who" triggered an event, so this is a real limitation being assumed away for now, not solved.
- **False positive vs. missed-fall tolerance — proposed default, unconfirmed.** Lean toward over-alerting rather than missing a real fall: this is framed throughout as an experimental, non-life-safety-certified detector where a human reviews any alert, so the cost of a false alarm (annoyance) is much lower than the cost of a missed fall. This is already the implicit shape of the scoring in `fusion.py`/`thresholds.py` (e.g. `confirm_score` is reachable well before the full verification window elapses) — stating it explicitly so future threshold tuning has a stated target to tune toward, rather than each change being ad hoc.

All four "proposed default" items above are working assumptions Claude made to keep moving, not decisions — worth a quick explicit yes/no from you before they're relied on further, especially the toolchain choice.
...
8. ~~Edge-case tests stressing threshold boundaries~~ — done (`tests/test_fusion_edge_cases.py`). This caught and fixed a real gap: sustained, radar-uncorroborated audio noise (TV, alarm) could previously rack up unbounded score and false-confirm on audio alone. Fixed by capping how many times a bare (unaligned) impulse can add its base weight per window (`audio_impulse_cap` in `thresholds.py`) — now confirming always needs at least some radar evidence too.
9. ~~Open questions~~ — given proposed default answers above (privacy handling, alert destination, occupant assumptions, false-positive/missed-fall philosophy), each explicitly marked unconfirmed. **Needs a real yes/no from you**, not just Claude's best guess, before relying on them further.
10. **Toolchain decision needed before firmware work starts**: Arduino, PlatformIO, or ESP-IDF for the ESP32-S3 port. This blocks step 11 below.
11. Wire `radar_protocol.py` / `audio_pcm.py` into an actual read loop once the ESP32-S3 + LD2410C + INMP441 hardware is assembled, and use it to settle the two unconfirmed decoder assumptions from step 7 against real captured frames.
12. Keep `config/hardware/` and `config/app/` as the home for threshold values and settings once the firmware port starts, per the structure already defined in their README files.
